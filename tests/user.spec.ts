...
describe("user", () => {
  ...
  describe("follows", () => {
    themedIt("getUserIsWatching", async (client, theme) => {
      const expectedSelfLink = `/watchlist/by/${ACCT_PROFILE_USER}/1/`;

      const actual = await client.getUserIsWatchingPage(ACCT_PROFILE_USER, 1);
      expect(actual.self_link).toEqual(expectedSelfLink);
      expect(actual.users).toBeDefined();
      expect(actual.users).toEqual(
        expect.arrayContaining([expect.objectContaining(expectedFollow)])
      );
    ...
    themedIt("getUserIsWatchedBy", async (client, theme) => {
      const expectedSelfLink = `/watchlist/to/${ACCT_PROFILE_USER}/1/`;

      const actual = await client.getUserIsWatchedByPage(ACCT_PROFILE_USER, 1);
      expect(actual.self_link).toEqual(expectedSelfLink);
      expect(actual.users).toBeDefined();
      expect(actual.users).toEqual(
        expect.arrayContaining([expect.objectContaining(expectedFollow)])
